...
print("Data size: ", str(data_size), "Train size: ", str(train_size), "Val size: ", str(val_size), "Test size: ",
      str(test_size))
num_classes = 86
...
dataset_val = torch.utils.data.Subset(dataset_test, indices[train_size:train_size + val_size])
dataset_test = torch.utils.data.Subset(dataset_test, indices[train_size + val_size:])
...
def evaluate_model(render, map, dataset_val):
    p = map.index(render)
    img, _ = dataset_val[p]
...
table = [["Trained on", train_size], ["Tested on", val_size], ["Objects", total], ["TP", total_tp], ["FN", total_fn],
         ["FP", total_fp],
...
for epoch in range(num_epochs):
    ...
    map = make_map(dataset_val)
    for i in range(len(map)):
        tot, tp, fp, fn = evaluate_model(map[i], map, dataset_val)
        total += tot
        total_tp += tp
        total_fp += fp
        total_fn += fn
    precision_val.append([total, total_tp, total_fp, total_fn])
    recall_val.append(calculate_recall(total_tp, total_fn))
...
## SAVE TRAINED MODEL
torch.save(model.state_dict(), 'faster-rcnn-cp-val.pth')

t5 = time.time()

with open('Faster-rcnn-CP.txt', 'w+') as file:
    file.write(str([(t1 - t0) / 60, (t2 - t1) / 60, (t3 - t2) / 60, (t4 - t3) / 60, (t5 - t4) / 60]))
    file.write("\n")
    file.write("VALIDATION")
    file.write(str(precision_val))
    file.write("\n")
    file.write(str(recall_val))
    file.close()
