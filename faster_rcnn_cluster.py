...
614321,623224,654126,4109995,4114319,4118793,4121739,4121967,4143562,4153825,4153827,4157124,4160403,4165967,
4181134,4181135,4181144,4210718,4211054,4211149,4211166,4211201,4211221,4211412,4211437,4211511,4211614,
4212454,4504369,4504381,4517992,4520632,4527943,4529240,4529247,4539099,4548180,4550325,4550348,4558957,
4567338,4621545,4639693,4647553,4649749,4650630,4651236,4655172,6003007,6004938,6022023,6022064,6025026,
6028324,6035291,6062574,6069259,6073026,6074890,6075075,6075079,6104154,6138173,6151663,6177697,6223631,
...
class PennFudanDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        img_path = os.path.join(self.root, "CroppedRenders/crender" + str(idx) + ".png")
        img = Image.open(img_path).convert("RGB")
        
        annotation = pd.read_csv(os.path.join(self.root, 'CroppedAnnotations/crender' + str(idx) + '.csv'))
        ...
        for i in range(num_objs):
            ...
            boxes.append([np.minimum(xmin, xmax), np.minimum(ymin, ymax), np.maximum(xmin, xmax), np.maximum(ymin, ymax)])
...
data_loader = torch.utils.data.DataLoader(
    dataset, batch_size=10, shuffle=True, num_workers=10,
    collate_fn=utils.collate_fn)

data_loader_test = torch.utils.data.DataLoader(
    dataset_test, batch_size=10, shuffle=False, num_workers=10,
    collate_fn=utils.collate_fn)
...
for epoch in range(num_epochs):
    dict_losses['Epoch' + str(epoch)] = train_one_epoch_test(model, optimizer, data_loader, device, epoch, print_freq=10)
...
def evaluate_model(render):
    ...
    groundtruth = pd.read_csv(root + '/CroppedAnnotations/crender' + str(render) + '.csv')
